...
const ChatBox = () => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-100 flex items-center justify-center sm:p-4">
      <div className="bg-white sm:rounded-xl shadow-2xl w-full max-w-2xl h-screen sm:h-[500px] flex flex-col">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-100">
          {isLoading ? (
            ...
            messages.map((message) => (
              ...
              >
                <div
                  className={`max-w-[70%] rounded-lg p-4 ${message.sender_id === user.id ? "bg-indigo-600 text-white" : "bg-white border border-gray-200 text-gray-800"}`}
                >
        ...
        {chat?.listing?.status === "active" ? (
          ...
          >
            <div className="flex items-end space-x-2">
              ...
              />
              <button
                className="bg-indigo-600 hover:bg-indigo-700 text-white p-2.5 rounded-full disabled:opacity-50 transition-colors"
                type="submit"
          ...
          <div className="p-4 bg-white border-t border-gray-200 rounded-b-lg">
            <p className="text-sm text-gray-600 text-center">
              {chat ? `Listing is ${chat?.listing?.status}` : "Loading..."}
            </p>
          </div>
        )}
      </div>
